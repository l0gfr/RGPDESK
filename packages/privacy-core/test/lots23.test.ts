...
describe("documentary history", () => {
  ...
  it("explicitly migrates v1 without modifying original IDs, revision or source", () => {
    ...
    expect(migrated.format).toBe("rgpd-master-v6"); expect(migrated.revision).toBe(m.revision); expect(migrated.activities[0]?.id).toBe(m.activities[0]?.id); expect(JSON.stringify(legacy)).toBe(bytes);
