...
describe("internal analysis and explicitly declared flows", () => {
  it("starts with unknown facts, no flow or legal decision, and validates the current version", () => {
    ...
    expect(master.format).toBe("rgpd-master-v10"); expect(activity.flows).toEqual([]);
  ...
  it("migrates v2 in memory preserving existing content, IDs, revision and historical deliveries", () => {
    ...
    const previous = JSON.parse(JSON.stringify(master)); previous.format = "rgpd-master-v2"; delete previous.impactAssessments; delete previous.dpoCases; delete previous.piaPublications;
