...
export function projectShare(master: Workspace, options: ShareOptions, id: () => string, now: string): SharedRegister {
  ...
  const internalIds = new Set([master.id, ...master.impactAssessments.flatMap((p) => [p.id, ...p.reviews.flatMap((r) => [r.id, r.context.activity.id, ...r.context.activity.flows.map((f) => f.id), ...(r.context.activity.role === "controller" ? r.context.activity.purposes.map((purpose) => purpose.id) : []), ...r.context.parties.map((item) => item.id), ...r.context.systems.map((item) => item.id), ...r.context.documents.map((item) => item.id), ...r.content.risks.map((item) => item.id), ...r.content.measures.map((item) => item.id), ...r.content.alternatives.map((item) => item.id)]), ...p.content.risks.map((r) => r.id), ...p.content.measures.map((m) => m.id), ...p.content.alternatives.map((a) => a.id)]), ...master.activities.flatMap((a) => a.flows.map((f) => f.id)), ...master.activities.map((a) => a.id), ...master.parties.map((p) => p.id), ...master.documents.map((d) => d.id), ...master.systems.map((s) => s.id), ...master.activities.flatMap((a) => a.role === "controller" ? a.purposes.map((p) => p.id) : [])]);
