...
export function reviseWorkspace(master: Workspace, expectedRevision: number, now: string, changes: Partial<Pick<Workspace, "organization" | "scope" | "jurisdiction" | "parties" | "systems" | "activities" | "documents" | "decisions" | "actions" | "imports" | "deliveries">>): Workspace {
  assertWorkspace(master);
  const allowed = new Set(["organization", "scope", "jurisdiction", "parties", "systems", "activities", "documents", "decisions", "actions", "imports", "deliveries"]);
