...
export function activityFacts(a: Activity, inventory?: Pick<Workspace, "parties" | "systems">) {
  ...
  const recipients = a.flows.filter(f => f.dataGroupIds?.length).map(f => {
    const ref = f.destinationRef;
    const name = ref === "subjects" ? knowledgeText(flowSubjects(a, f)) : ref ? inventory && (ref.startsWith("party:") ? inventory.parties : inventory.systems).find(e => e.id === ref.slice(ref.indexOf(":") + 1))?.name : knowledgeText(f.destination);
    // A tool destination alone is not a recipient. The declared access identifies who can consult.
    return [ref?.startsWith("system:") ? "" : name, knowledgeText(f.access)].filter(Boolean).join(" · ");
  });
  return { dataCategories: category("data", a.dataCategories), dataSubjects: category("people", a.dataSubjects), recipients: groups.some(g => !a.flows.some(f => f.dataGroupIds?.includes(g.id)) || a.flows.some(f => f.dataGroupIds?.includes(g.id) && f.access.state === "unknown")) ? unknown() : join([knowledgeText(a.recipients), ...recipients]) };
}
export function purposeRetention(a: Activity, p: Purpose): Purpose["retention"] {
  const groups = (a.dataGroups ?? []).filter(g => g.purposeIds.includes(p.id));
...
export function dataGroupIssues(a: Activity, g: DataGroup): { key: string; message: string }[] {
  ...
  if (a.role === "controller" && !g.purposeIds.length) add("purposes", "Relier une sous-finalité.");
  ...
  if (flows.some(f => (!f.sourceRef && f.source.state === "unknown") || (!f.destinationRef && f.destination.state === "unknown") || f.channel.state === "unknown" || f.access.state === "unknown")) add("flow-details", "Préciser l’origine, la destination, les supports, canaux et accès.");
...
export function dataGroupRows(a: Activity, inventory: Pick<Workspace, "parties" | "systems">): { group: string; rows: { label: string; value: string }[] }[] {
  ...
  return (a.dataGroups ?? []).map(g => ({ group: `D${g.code}`, rows: [
    ...
    { label: "Sous-finalités", value: a.role === "controller" ? g.purposeIds.map(id => text(a.purposes.find(p => p.id === id)?.description)).join(" ; ") || "À relier" : "Opérations du client : " + text(a.operations) },
    ...a.flows.filter(f => f.dataGroupIds?.includes(g.id)).flatMap((f, i) => [
