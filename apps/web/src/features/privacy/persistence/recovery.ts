import { canonicalJson, validateRecoveryDraft, parseBoundedJson, PrivacyError, type Activity, type Workspace, type DpoCase, type DpoReview, type ImpactAssessment, type PiaReview, type EvidenceReference, type Party, type System } from "@rgpdesk/privacy-core";
...
export interface RecoveryDraft {format:'rgpd-draft-v1';workspaceId:string;revision:number;id:string;sequence:number;updatedAt:string;form:RecoveryForm}
...
export function parseRecovery(value:unknown):RecoveryDraft {
  ...
  if(!validateRecoveryDraft(bounded))throw new PrivacyError('INVALID');
