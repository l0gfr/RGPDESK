<script lang="ts">
  ...
  let { workspace, busy, onDeliver, onDownload }: { workspace: Workspace; busy: boolean; onDeliver: (prepared: PreparedDelivery) => Promise<void>; onDownload: (id: string) => Promise<void> } = $props();
...
<section class="panel">
  ...
  {#if !prepared}<form onsubmit={(e) => { e.preventDefault(); void prepare(); }}><fieldset disabled={busy || preparing}>
    ...
    <p class="help">L’historique chiffré sera enregistré avant le téléchargement. Vérifiez ensuite le fichier sur votre disque. Au maximum 8 livraisons de 512 Kio dans cet espace.</p>
  {/if}
</section>
