...
<section class="panel" aria-label="Restitution AIPD">
  ...
  {#if !prepared}<form onsubmit={(e) => { e.preventDefault(); prepare(); }}><fieldset disabled={busy}>
    <label class="field">Étude à restituer<select bind:value={options.piaId} onchange={() => { options.reviewId = null; options.sections = []; }}><option value="">Choisir une étude</option>{#each workspace.impactAssessments.filter(p => p.scope !== "risks") as p}<option value={p.id}>{workspace.activities.find((a) => a.id === p.activityId)?.title}</option>{/each}</select></label>
