...
<section class="panel">
  ...
  {#if !workspace.documents.length && !draft}<div class="empty"><span class="icon-tile"><Icon name="documents" size={30} /></span><h3>Un dossier qui garde ses sources.</h3><p>Commencez par relier une référence à une activité et à son périmètre.</p></div>{/if}
  ...
  {#if draft}<form class="subpanel" onsubmit={(e) => { e.preventDefault(); void save(); }}><fieldset disabled={busy}>
    ...
    <label class="field">Localisation / référence interne<textarea maxlength="4000" bind:value={draft.internalRef}></textarea><small>Texte inerte, jamais ouvert automatiquement et exclu des livraisons.</small></label>
