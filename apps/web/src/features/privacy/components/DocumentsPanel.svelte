...
<section class="panel">
  ...
  <ul class="records">{#each workspace.documents as doc}<li><span class="record-icon"><Icon name="documents" /></span><div class="grow"><strong>{doc.title}</strong>{#if doc.documentCode}<span class="document-code">{doc.documentCode}</span>{/if}<p>{categories[doc.category]} · {doc.scope} · {doc.status === "reviewed" && doc.reviewedRevision === workspace.revision && (!doc.reviewDue || doc.reviewDue > new Date().toISOString().slice(0, 10)) ? "Revue déclarée" : "À réexaminer / déclaré"}</p></div><button class="secondary" disabled={busy} onclick={() => onLeave(()=>edit(doc))}>Examiner {doc.title}</button></li>{/each}</ul>
  {#if draft}<form class="subpanel" onsubmit={(e) => { e.preventDefault(); void save(); }}><fieldset disabled={busy}>
    ...
    {#key draft.id}<DocumentFiling bind:draft documents={workspace.documents} />{/key}
