...
<section id="workspace-search" class="workspace-search" aria-labelledby="search-title">
  ...
  <div class="search-controls"><label class="field"><span>Rechercher dans ce coffre</span><span class="search-input"><Icon name="search" size={24} /><input bind:this={input} type="search" bind:value={query} maxlength="160" autocomplete="off" spellcheck="false" placeholder="Une activité, un prestataire, une question…" onkeydown={(event) => { if (event.key === "Escape") onClose(); }} /></span></label><label class="field">Dans<select bind:value={kind}><option value="">Toutes les rubriques</option>{#each Object.entries(SEARCH_LABELS) as [value, label]}<option {value}>{label}</option>{/each}</select></label></div>
