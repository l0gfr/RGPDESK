<script lang="ts">
  ...
  let { workspace, busy, onSave, onEditing, onRegister }: { workspace: Workspace; busy: boolean; onSave: (next: Workspace) => Promise<boolean>; onEditing: (editing: boolean) => void; onRegister: () => void } = $props();
...
<section class="panel pia-workbench" aria-label="Atelier AIPD">
  ...
  {:else if context}
    <div class="section-heading"><div><p class="eyebrow">Atelier AIPD · {context.activity.role === "processor" ? "Contribution au dossier du responsable" : "Dossier du responsable"}</p><h2>{context.activity.title}</h2></div><span class="pia-save-state">{dirty ? "Modifications à enregistrer" : "Étude enregistrée dans le coffre"}</span></div>
    <p class="help">Enregistrez avant de verrouiller le coffre. Les modifications non enregistrées seront abandonnées. {context.activity.role === "processor" ? "L’assistance du sous-traitant ne remplace pas la décision du responsable (article 28 §3 f)." : ""}</p>
    ...
    {:else}
      ...
      <p class="help">Ce dossier reste interne au coffre et à sa sauvegarde chiffrée. Les exports de registre n’incluent pas les notes AIPD. Il n’existe pas encore d’échange de fichiers avec le logiciel PIA de la CNIL.</p>
    {/if}
    <div class="actions pia-toolbar"><button onclick={() => void save()}>{busy ? "Chiffrement…" : "Enregistrer l’étude"}</button>{#if step < PIA_STEPS.length - 1}<button class="secondary" onclick={() => void go(step + 1)}>Étape suivante<Icon name="arrow" /></button>{/if}<button class="secondary" onclick={close}>{dirty ? "Quitter sans enregistrer" : "Fermer l’étude"}</button></div>
    </fieldset>
  {/if}
</section>
