...
<details class="review-evidence">
  ...
  <div class="review-evidence-body">
    ...
    {#each unique as doc}<article><header><Icon name="documents" /><div><h4>{doc.title}</h4>{#if doc.documentCode}<p>{doc.documentCode}</p>{/if}<p>{doc.version ? `Version ${doc.version}` : "Version non renseignée"} · {doc.status === "reviewed" ? "Revue déclarée" : "Déclarée, à examiner"}</p></div></header><dl><div><dt>Périmètre</dt><dd>{doc.scope || "À préciser"}</dd></div><div><dt>Où le retrouver</dt><dd>{doc.internalRef || "Localisation non renseignée"}</dd></div><div><dt>Réserves</dt><dd>{doc.reservations || "Aucune réserve saisie"}</dd></div>{#if doc.fingerprint}<div><dt>Version repérée par empreinte</dt><dd>{doc.fingerprint.version} · SHA-256 · {doc.fingerprint.bytes.toLocaleString("fr-FR")} octets. Le fichier original reste à conserver séparément.</dd></div>{/if}{#if doc.reviewedAt}<div><dt>Examen déclaré</dt><dd>{doc.reviewedAt.slice(0,10)}</dd></div>{/if}{#if doc.reviewDue}<div><dt>Prochain examen</dt><dd>{doc.reviewDue}</dd></div>{/if}</dl>{#if !historical && onOpen}<button type="button" class="text-button" {disabled} onclick={() => onOpen?.(doc.id)}>Examiner {doc.title}<Icon name="arrow" size={16} /></button>{/if}</article>{/each}
