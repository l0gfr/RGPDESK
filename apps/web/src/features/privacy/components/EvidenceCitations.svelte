...
<div class="evidence-citations">
  {#each citations ?? [] as citation, index}
    ...
    <article class="citation-card">
      <header>{#if document?.documentCode}<span>{document.documentCode}</span>{/if}<strong>{document?.title || "Référence hors de ce périmètre"}</strong><span>Version citée : {citation.version || "non précisée"}</span></header>
  ...
  {/each}
  {#if !readonly && documents.length}<div class="citation-add"><label class="field">{prefix} · Citer un justificatif lié<select bind:value={chosen}><option value="">Choisir une référence</option>{#each documents as doc}<option value={doc.id}>{doc.documentCode ? `${doc.documentCode} · ` : ""}{doc.title} · {doc.version || "version à préciser"}</option>{/each}</select></label><button type="button" class="secondary" disabled={!chosen || (citations?.length ?? 0) >= 8} onclick={add}>Citer un passage</button></div><p class="help">La référence reste commune. Vous précisez ici seulement le passage utile à cette question et ce qu’il permet d’établir. Les fichiers restent dans votre organisation.</p>{/if}
</div>
