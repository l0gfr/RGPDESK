...
<section class="panel analysis-overview">
  ...
  {#if activity}
    ...
    {#if mode === "flows"}
      {#if activity.flows.length}<FlowMap activity={activity} inventory={workspace} flows={resolvedFlows(activity, workspace)} />{:else}<div class="flow-empty"><Emblem name="flows" /><h3>Les flux de cette activité restent à décrire.</h3><p>Le lien avec un prestataire ou un logiciel ne renseigne pas, à lui seul, la circulation des données.</p></div>{/if}
    {:else}
