...
<section class="panel analysis-overview">
  ...
  {#if activity}
    ...
    <nav class="analysis-lenses" aria-label="Quatre lectures du même inventaire">
      <button class="secondary" disabled={busy} onclick={() => onEdit(activity!, "analysis")}><span>01 / RGPD</span><strong>Exigences du traitement</strong><small>Finalités, fondements, droits et sécurité</small></button>
      <button class="secondary" disabled={busy} onclick={() => onPia(activity!.id, 2)}><span>02 / Nécessité & proportionnalité</span><strong>Éprouver les choix</strong><small>Options et atteintes, dans l’atelier AIPD</small></button>
      <button class="secondary" disabled={busy} onclick={() => onSecurity(activity!.id)}><span>03 / Risques SI</span><strong>Protéger l’activité</strong><small>Événements, dépendances et mesures</small></button>
      <button class="secondary" disabled={busy} onclick={() => onPia(activity!.id, 3)}><span>04 / Droits & libertés</span><strong>Examiner les effets humains</strong><small>Scénarios et garanties, dans l’atelier AIPD</small></button>
    ...
    {#if mode === "flows"}
      {#if activity.flows.length}<FlowMap flows={resolvedFlows(activity, workspace)} />{:else}<div class="flow-empty"><Icon name="flows" size={54} /><h3>Les flux de cette activité restent à décrire.</h3><p>Le lien avec un prestataire ou un logiciel ne renseigne pas, à lui seul, la circulation des données.</p></div>{/if}
    {:else}
