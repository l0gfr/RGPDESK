...
{#if step}<aside class="demo-guide" aria-label="Repère de démonstration"><span class="demo-guide-number">{String(index + 1).padStart(2, "0")}<small>/ 08</small></span><div><p class="eyebrow">Dans cet exemple</p><h2>{step.title}</h2><p>{step.text}</p></div><button class="secondary" disabled={busy} onclick={() => onNavigate(next?.panel ?? "overview")}>{next ? "Étape suivante" : "Retour à la visite"}<Icon name="arrow" size={17} /></button></aside>{/if}
