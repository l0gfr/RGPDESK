<script lang="ts">
  ...
  import DeskIllustration from "./DeskIllustration.svelte";
...
<div data-rgpdesk-ready={ready ? "true" : "false"} class="privacy-app">
  ...
  {#if master}
    ...
    <header class="workspace-heading"><div><p class="eyebrow">Dossier privé / révision {master.revision}</p><h1>{panel === "register" ? "Le registre de vos activités." : panel === "documents" ? "Une documentation reliée." : panel === "actions" ? "Le suivi, dans la durée." : panel === "delivery" ? "Un partage à votre mesure." : panel === "import" ? "Vos données prennent place." : panel === "backup" ? "La continuité de votre dossier." : panel === "organization" ? "Votre organisation." : panel === "parties" ? "Les acteurs du traitement." : "Les moyens du traitement."}</h1></div><button class="secondary lock-button" onclick={() => lock()}><Icon name="lock" />Verrouiller le coffre</button></header>
    ...
    {:else if panel === "register"}
      ...
      <section class="panel"><div class="section-heading"><div><p class="eyebrow">Documenter les traitements</p><h2>Votre registre</h2></div><span class="tag">Aucune conclusion automatique</span></div>
        ...
        {#if master.activities.length === 0}<div class="empty register-empty"><span class="icon-tile"><Icon name="register" size={35} /></span><h3>Le premier chapitre de votre registre.</h3><p>Décrivez une activité, ses finalités et ce qui reste à examiner.<br />Chaque information trouve sa place, chaque inconnue reste visible.</p><button class="text-button" onclick={() => panel = "import"}>Explorer un modèle fictif <Icon name="arrow" size={16} /></button></div>{/if}
  ...
  {:else}
    <div class="welcome-hero"><header class="intro"><p class="eyebrow"><span class="eyebrow-line"></span>Le bureau privé de vos données</p><h1>La rigueur du registre.<br /><em>La discrétion d’un coffre.</em></h1><p>Un lieu pour documenter vos traitements, organiser vos décisions et partager des dossiers examinés. Vos données restent sur votre appareil.</p><div class="trust-row"><span><Icon name="lock" size={17} />Coffre chiffré</span><span><Icon name="shield" size={17} />Sans compte</span><span><Icon name="documents" size={17} />Partage choisi</span></div></header><DeskIllustration /></div>
    <div class="grid-two vault-panels">
      <section class="panel"><p class="eyebrow">01 / Nouveau coffre</p><h2>Créer un espace</h2>
