<script lang="ts">
  ...
  let searchCanNavigate = $derived(!busy && ["overview", "register", "analysis", "flows", "pia", "risks", "dpo"].includes(panel));
  ...
  const currentGuard=():DraftGuard|undefined=>editor?activityGuard:(panel==='pia'||panel==='risks')?piaGuard:panel==='dpo'?dpoGuard:panel==='documents'?documentGuard:panel==='parties'||panel==='systems'?relationGuard:panel==='organization'?organizationGuard:panel==='actions'?actionsGuard:panel==='overview'?reexaminationGuard:undefined;
  ...
  function resumeCheckpoint(){
    ...
    requestNavigation(()=>{if(!master)return;resetEditor();searchNavigation++;
      ...
      else if(c.kind==="pia"){piaActivityId=master.impactAssessments.find(p=>p.id===c.id)?.activityId??"";piaStep=c.step;lensPia=true;panel=master.impactAssessments.find(p=>p.id===c.id)?.scope === "risks" ? "risks" : "pia";}
  ...
  async function openSearchResult(result: SearchResult) {
    ...
    editorSection = "record"; editorExample = undefined;
      ...
      if (!pia) return;
      piaActivityId = pia.activityId; panel = pia.scope === "risks" ? "risks" : "pia";
    }
    });
  }
...
<div data-rgpdesk-ready={ready ? "true" : "false"} class="privacy-app">
  ...
  {#if master}
    ...
    <aside class="desk-sidebar"><div class="sidebar-caption"><span class="workspace-avatar">{master.organization.name.slice(0, 1).toUpperCase()}</span><div><small>{demo ? "DOSSIER FICTIF" : "VOTRE ESPACE"}</small><h2>{master.organization.name}</h2><small>{master.activities.length} fiche(s) dans votre registre</small></div></div>
      ...
      {#each [["overview", "Ma mission"], ["register", "Registre"], ["flows", "Cartographie"], ["analysis", "Analyse"], ["risks", "Risques & mesures"], ["pia", "AIPD / PIA"], ["dpo", "Dossiers DPO"], ["organization", "Organisation"], ["parties", "Intervenants"], ["systems", "Systèmes"], ["documents", "Documents"], ["actions", "Actions & décisions"]] as [key, label]}
        <button aria-label={label} class:active={panel === key} aria-current={panel === key ? "page" : undefined} disabled={busy} onclick={() => navigateTo(key as typeof panel)}><Icon name={key === "risks" ? "security" : key === "pia" ? "impact" : key === "dpo" ? "briefcase" : key === "pia-sharing" ? "delivery" : key} /><span>{label}</span>{#if key === "register"}<small>{master.activities.length}</small>{/if}</button>
