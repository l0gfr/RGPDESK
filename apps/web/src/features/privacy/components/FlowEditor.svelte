<script lang="ts">
  ...
  let { flows = $bindable(), activity, workspace, compact = false, groupId = "" }: { flows: DataFlow[]; activity: Activity; workspace: Workspace; compact?: boolean; groupId?: string } = $props();
  ...
  function addFlow() { if (flows.length >= 20) return; const id = crypto.randomUUID(); flows = [...flows, { ...createDataFlow(id), ...(groupId ? { dataGroupIds: [groupId] } : {}) }]; void edit(id); }
...
<div class="flow-editor" bind:this={root}>
  ...
  {#each flows as flow, index (flow.id)}
    ...
    {/if}
  {/each}
  {#if !groupId && flows.length}<h3>Votre carte, d’après ces déclarations</h3><FlowMap flows={resolvedFlows({ ...activity, flows }, workspace)} onEdit={(id) => void edit(id)} />{/if}
