<script lang="ts">
  ...
  import { DECLARATIONS_CONTEXT, filterDeclarations, type DeclarationKind, type DeclarationSource, type ReusableDeclaration } from "../reusable-declarations";
  import Icon from "./Icon.svelte";
  let { kind, label, current, onUse }: { kind: DeclarationKind; label: string; current: string; onUse: (text: string) => void } = $props();
  ...
  let results = $derived(filterDeclarations(entries, query, current));
  async function toggle() {
    if (!open) { query = ""; pending = null; }
...
<details class="declaration-picker" bind:open ontoggle={toggle}>
  <summary bind:this={summary}><Icon name="history" size={17}/><span>Réutiliser une déclaration</span></summary>
  {#if open}
    <div class="declaration-body">
      <p class="declaration-help">Déclarations de ce coffre, y compris votre saisie en cours. Reprendre un texte crée une copie modifiable ; les autres fiches restent inchangées.</p>
