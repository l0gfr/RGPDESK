<script lang="ts">
  ...
  let { flows, onEdit }: { flows: DataFlow[]; onEdit?: (id: string) => void } = $props();
</script>
<ol class="flow-map" aria-label="Carte des flux déclarés">
  {#each flows as flow, index (flow.id)}
    <li class="flow-route">
      <div class="flow-route-bar"><span>FLUX {String(index + 1).padStart(2, "0")}</span>{#if onEdit}<button type="button" class="text-button" onclick={() => onEdit?.(flow.id)}>Modifier le flux {index + 1}</button>{:else}<span>Déclaration de votre organisation</span>{/if}</div>
