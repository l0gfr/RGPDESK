<script lang="ts">
  ...
  import { activityFacts, purposeRetention, resolvedFlows, knowledgeText, piaOpenPoints, type PiaContent, type PiaContext, type ReviewNote } from "@rgpdesk/privacy-core";
...
<article class="pia-dossier" aria-label="Dossier AIPD en lecture">
  ...
  <section class="pia-reader-section" hidden={chapter !== 0} aria-label={chapters[0].title}><dl><div><dt>Examen du champ applicable</dt><dd>{knowledgeText(content.applicability) || "À documenter"}</dd></div><div><dt>Position sur la réalisation de l’AIPD</dt><dd>{({ unknown: "Décision à instruire", required: "AIPD jugée requise", voluntary: "AIPD engagée volontairement", "not-required": "AIPD jugée non requise" })[content.screeningDecision]}</dd></div><div><dt>Motif de réalisation ou de non-réalisation</dt><dd>{knowledgeText(content.screeningReason) || "À documenter"}</dd></div><div><dt>Portée de l’organisation</dt><dd>{knowledgeText(context.scope) || "À documenter"}</dd></div><div><dt>Juridiction examinée</dt><dd>{knowledgeText(context.jurisdiction) || "À documenter"}</dd></div></dl>
    ...
    <dl>{#each [["Personnes", activityFacts(context.activity, context).dataSubjects], ["Données", activityFacts(context.activity, context).dataCategories], ["Destinataires", activityFacts(context.activity, context).recipients], ["Transferts", context.activity.transfers], ["Sécurité", context.activity.securityMeasures]] as [label, value]}<div><dt>{label}</dt><dd>{knowledgeText(value as typeof context.scope) || "À documenter"}</dd></div>{/each}</dl>
    {#if context.activity.role === "controller"}{#each context.activity.purposes as purpose, index}<h5>Finalité {index + 1}</h5><dl><div><dt>Objectif</dt><dd>{knowledgeText(purpose.description) || "À documenter"}</dd></div><div><dt>Fondement juridique</dt><dd>{knowledgeText(purpose.legalBasis) || "À documenter"}</dd></div><div><dt>Conservation / déclencheur</dt><dd>{knowledgeText(purposeRetention(context.activity, purpose).period) || "À documenter"} / {knowledgeText(purposeRetention(context.activity, purpose).trigger) || "À documenter"}</dd></div></dl>{/each}{:else}<dl><div><dt>Opérations confiées</dt><dd>{knowledgeText(context.activity.operations) || "À documenter"}</dd></div><div><dt>Instructions</dt><dd>{knowledgeText(context.activity.instructions) || "À documenter"}</dd></div></dl>{/if}
    ...
    {#if context.activity.flows.length}<FlowMap activity={context.activity} inventory={context} flows={resolvedFlows(context.activity, context)} />{:else}<p>Aucun flux décrit.</p>{/if}
