<script lang="ts">
  ...
  function support() {
    const name=newSupport.trim();if(!name || name.length>160 || (activity.flowSupports?.length ?? 0)>=20 || (activity.flowSupports ?? []).some(s=>s.code>=9999))return;
