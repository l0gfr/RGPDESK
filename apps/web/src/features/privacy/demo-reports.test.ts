...
test("the fixed final dossier includes both roles and every shareable PIA section without internal examples or active content", async () => {
  ...
  const piaHeadings = [...reports[2]!.html.matchAll(/<h2(?: id="pia-section-\d+-title")?>([^<]+)<\/h2>/g)].map(m => m[1]);
  for (const section of Object.values(PIA_SHARE_SECTIONS)) expect(piaHeadings).toContain(section);
