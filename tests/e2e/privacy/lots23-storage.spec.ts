...
test("IndexedDB v1 upgrade preserves the old encrypted master and v1 backup migration", async ({ page }) => {
  ...
  }); expect(result).toEqual({ untouched: true, format: "rgpd-master-v8", oldRevision: 1, same: true });
});
...
test("v2 encrypted content migrates without rewriting disk and v3 analysis survives backup restoration", async ({ page }) => {
  ...
  expect(result).toEqual({ untouched: true, format: "rgpd-master-v8", revision: 2, equal: true, leaked: false, sourceUnchanged: true });
});
...
test("a genuine v3 vault remains intact on opening and AIPD reviews survive native backup restore", async ({ page }) => {
  ...
  expect(result).toEqual({ untouched: true, equal: true, count: 1, format: "rgpd-master-v8", leaked: false });
});
...
test("v4 migrates read-only then DPO v5 reviews and publications survive native encrypted restore", async ({ page }) => {
  ...
  expect(result).toEqual({ untouched: true, equal: true, format: "rgpd-master-v8", leaked: false, cases: 4, reviews: 4, publications: 1, listed: 1 });
});
...
test("v5 encrypted vault migrates in memory then saves linked flows and v2 delivery without data loss", async ({page}) => {
 ...
 expect(result).toEqual({unchanged:true,format:"rgpd-master-v8",operation:{state:"documented",value:"PRIVATE_LINKED_OPERATION"},same:true,snapshots:1,valid:true,leak:false,version:"rgpd-share-v2"});
});
...
test("v6 documentary vault opens without writing and v7 metadata survives encrypted backup and restore", async ({page}) => {
 ...
 expect(result).toEqual({unchanged:true,format:"rgpd-master-v8",equal:true,code:"DOC-0001",fingerprint:{algorithm:"SHA-256",sha256:"b".repeat(64),bytes:3,capturedAt:expect.any(String),version:"v1"},leak:false});
});
