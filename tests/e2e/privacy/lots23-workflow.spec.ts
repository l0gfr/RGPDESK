...
test("a change from another tab invalidates the already reviewed delivery", async ({ page, context }) => {
  ...
  await other.getByRole("button", { name: "Registre", exact: true }).click();
  await other.getByRole("button", { name: "Modifier Bénévoles • exemple fictif", exact: true }).click(); await other.getByRole("button", { name: "Voir toute la fiche", exact: true }).click(); await other.getByLabel("Notes internes", { exact: true }).fill("Changement interne fictif invalidant aussi la revue"); await other.getByRole("button", { name: "Enregistrer la fiche", exact: true }).click(); await expect(other.getByRole("button", { name: "Modifier Bénévoles • exemple fictif", exact: true })).toBeVisible();
