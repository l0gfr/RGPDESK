...
test("real IndexedDB opens v8 without writing, stores v9 groups and round-trips its encrypted backup",async({page})=>{
 ...
 expect(result.unchanged).toBe(true);expect(result.format).toBe("rgpd-master-v11");expect(result.same).toBe(true);expect(result.raw+result.backup).not.toContain("CANARY");
});
